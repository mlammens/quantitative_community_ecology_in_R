...
Many analyses in quantiative community ecology rely on multivariate statistical approaches, such as PCA, RDA, and NMDS. These analyses require complete data sets, and will not work if there are missing values. Below we go through an example of how to search for and replace a missing value. **NOTE: data imputation (i.e., replacing missing values with approximate values) is an area of study all by itself and we will not be covering it here.**
