# Community Data {#comm-data}

## General structure of community ecology datasets
...
## Today's example dataset - Mafragh, Algeria vegetation data

We will use one main dataset throughout this tutorial. 
You should have loaded this dataset in the **Get the data** sub-section of Section \@ref(prereq), but we'll load the data again below.
...
> **NOTE:** the information in the `veg` object are derived from a dataset included in the `ade4` package. You can load the data from `ade4` with the code below and use the `?` function to access included documentation about these data. However, the data tables have different names in the included data than what we are working with today.
...
While we went over how to read in data via a \*.csv file in Section \@ref(intro-to-RStudio), we will take advantage of the fact that our example datasets have already been read in to R and saved as a \*.rda object.
